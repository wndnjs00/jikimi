...
//    private val storage: FirebaseStorage
    ...
    override suspend fun signup(email: String, password: String, nickname: String): Resource<User> = withContext(Dispatchers.IO) {
        try {
            // 사용자 생성(firebase를 통해 회원가입)
            val authResult = firebaseAuth.createUserWithEmailAndPassword(email, password).await()   // await를 통해 비동기작업이 완료될때까지 대기 / 성공시, authResult에 사용자의 인증정보가 담김
            // 사용자의 uid가져오기 (authResult.user?.uid를 통해 생성된 사용자의 고유 식별자(UID)를 가져옴)
            val uid = authResult.user?.uid ?: return@withContext Resource.Error("회원가입 실패: 사용자 정보를 가져올 수 없습니다.")

            val user = User(uid, email, nickname)
            // firestore에 "users"컬렉션에 사용자정보 저장
            firestore.collection("users").document(uid).set(user).await()   // Firestore 데이터베이스의 users 컬렉션에 해당 사용자의 uid를 키로 하는 문서를 생성하고,사용자 데이터를 저장 / await()를 사용하여 Firestore 작업이 완료될 때까지 대기
    ...
    override suspend fun login(email: String, password: String): Resource<User> = withContext(Dispatchers.IO) {
        try {
            // firebase를 통해 로그인 시도
            val authResult = firebaseAuth.signInWithEmailAndPassword(email, password).await()   // await를 통해 비동기작업이 완료될때까지 대기 / 성공시, authResult에 사용자의 인증정보가 담김
            // 사용자의 uid가져오기 (authResult.user?.uid를 통해 생성된 사용자의 고유 식별자(UID)를 가져옴)
            val uid = authResult.user?.uid ?: return@withContext Resource.Error("로그인 실패: 사용자 정보를 가져올 수 없습니다.")
            ...
            val user = userSnapshot.toObject(User::class.java) ?: return@withContext Resource.Error("사용자 정보를 가져올 수 없습니다.")
