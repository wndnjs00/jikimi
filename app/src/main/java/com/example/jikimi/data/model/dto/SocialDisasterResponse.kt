...
@Xml(name = "response")
data class SocialDisasterResponse(
    @Element(name = "header") val header : SocialHeader?,
    @Element(name = "body") val body : SocialBody?,
)


@Xml(name = "header")
data class SocialHeader(
...
data class SocialBody(
    @Element(name = "items") val items : SocialItems?,
...
@Parcelize
@Xml(name = "items")
data class SocialItems(
    @Element(name = "item") val item: List<SocialItem>?
) : Parcelable
